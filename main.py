...
def parser_commit_change(list_code_change):
    ...
    for change in list_code_change:
        ...
        for line in change:
            ...
            new_line = [new_line.split(">")[0] + ">"] + word_tokenize(" ".join(re.split(f"../{NAME_DIR}", new_line.split(">")[1])))
...
def read_commit_blame():
    ...
    dataset = pd.read_csv(
        str(pathlib.Path(
            __file__).parent.resolve()) + rf"/Data/{NAME_PROJECT}/blame/pydriller_{NAME_PROJECT}_bugfixes_bic.csv",
        delimiter=',')
...
def get_modification_transformation_data():
    for k in range(1, NUMBER_FOLD + 1):
        name_transformation = ['BooleanExchange', 'LoopExchange', 'PermuteStatement', 'ReorderCondition', 'SwitchToIf',
        ...
        for dir_transformation in name_transformation:
            ...
            if dir_transformation == NAME_PROJECT or dir_transformation == "":
                continue
            if not os.path.exists(f'../JavaTransformer/diff_after_transformation/{NAME_PROJECT}/{str(k)}/{dir_transformation}/transform.csv'):
                ...
                for i in transform_dir:
                    merge.append(pd.read_csv(f'../JavaTransformer/diff_after_transformation/{NAME_PROJECT}/{str(k)}/{dir_transformation}/{i}', engine ='python', delimiter="!!!!!#$$$", header=None))
                pd.concat(merge).to_csv(f'../JavaTransformer/diff_after_transformation/{NAME_PROJECT}/{str(k)}/{dir_transformation}/transform.csv', header=None, index=False)
            data = pd.read_csv(os.path.join(f'../JavaTransformer/diff_after_transformation/{NAME_PROJECT}/{str(k)}/{dir_transformation}/transform.csv'),
...
def read_ids_test(number_fold=1):
    data = pickle.load(open(os.path.join("./Data", NAME_PROJECT, str(number_fold), f"{NAME_PROJECT}_test.pkl"), 'rb'))
...
if __name__ == '__main__':
    ...
    name_projects = ['commons-lang', 'manifoldcf', 'openwebbeans', 'zookeeper', 'commons-collections', 'tapestry-5',
                     'knox', 'xmlgraphics-batik', 'deltaspike', 'zeppelin', 'kafka',  'jspwiki']
    url_projects = ['https://github.com/apache/commons-lang', 'https://github.com/apache/manifoldcf',
                     'https://github.com/apache/openwebbeans', 'https://github.com/apache/zookeeper',
                    'https://github.com/apache/commons-collections', 'https://github.com/apache/tapestry-5',
                    'https://github.com/apache/knox', 'https://github.com/apache/xmlgraphics-batik',
                    'https://github.com/apache/deltaspike', 'https://github.com/apache/zeppelin',
                    'https://github.com/apache/kafka', 'https://github.com/apache/jspwiki']
